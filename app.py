...
def summary_breakdown(breakdown_type):
    ...
    if breakdown_type == 'new_investment':
        ...
        for h in histories:
            # กรองเฉพาะประวัติที่มีตัวเงินจริงๆ (ดอกเบี้ย, ค่าปรับ, ยอดจ่าย, หรือส่วนลด > 0)
            if h.interest_paid > 0 or h.fine_amount > 0 or h.pay_amount > 0 or h.discount_amount > 0:
                ...
                <tr>
                    ...
                    <td class='text-center'><a href='/delete_history/{h.id}' class='btn btn-sm btn-danger' onclick="return confirm('ยืนยันการลบประวัติรายการนี้? (ระบบจะคืนยอดเงินต้นและดอกเบี้ยกลับให้อัตโนมัติ)')">❌ ยกเลิก</a></td>
                </tr>
                """

        # แสดงเฉพาะส่วนต่างยอดค้างเก่าที่มากกว่า 0 บาท
