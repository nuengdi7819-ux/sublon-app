...
def index():
    ...
    # คำนวณกำไรสะสมจากยอดรับชำระจริงทั้งหมด (รวมเงินต้นที่รับคืน + ดอกเบี้ย + ค่าปรับ)
    total_profit = db.session.query(db.func.sum(PaymentHistory.pay_amount + PaymentHistory.fine_amount)).scalar() or 0.0
